...
#-----------------------------------------------
	...
	while (<CSDDRD_DATA>) {
		...
		CHECK_CITY: foreach my $location (1..$#climate_ref) {
			if (($climate_ref[$location][0] =~ /$CSDDRD[4]/) && ($climate_ref[$location][1] =~ /$CSDDRD[3]/)) {
				& simple_replace ($hse_file[$extensions{cfg}], "#LAT", 1, "$climate_ref[$location][2] $climate_ref[$location][3]");
				& simple_replace ($hse_file[$extensions{cfg}], "#CLIMATE", 1, "*clm ../../../climate/$climate_ref[$location][4]");
				last CHECK_CITY;
			}
		}
