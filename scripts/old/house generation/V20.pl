...
#		else { $record_indc->{"main.tmc"} = 0; };
		...
		& simple_replace ($hse_file->[$record_extensions->{"cfg"}], "#SIM_SETUP", 1, 2, "*sps 1 10 1 10 1 0");	# sim setup: no. data sets retained; startup days; zone_ts (step/hr); plant_ts (step/hr); ?save_lv @ each zone_ts; ?save_lv @ each zone_ts;
		...
		& simple_replace ($hse_file->[$record_extensions->{"cfg"}], "#SIM_SETUP", 1, 4, "*sblr ../$hse_type-$hse_names{$hse_type}/$region_names{$region}/$CSDDRD->[1]/$CSDDRD->[1].res");	#res file path
