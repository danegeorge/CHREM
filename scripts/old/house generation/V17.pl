...
# 		#-----------------------------------------------
		...
		foreach my $zone (keys %{$zone_indc}) { 
			& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#ZONE_NAME", 1, 1, "GEN $zone This file describes the $zone");

			my $side_length = $CSDDRD->[100] ** 0.5;
			my $height;
			my $vert_offset;
			if ($zone eq "main") { $height = $CSDDRD->[112] + $CSDDRD->[113] + $CSDDRD->[114]; $vert_offset = 0;}
			elsif ($zone eq "bsmt") { $height = $CSDDRD->[109]; $vert_offset = -$height;}
			elsif ($zone eq "crwl") { $height = $CSDDRD->[110]; $vert_offset = -$height;}
			elsif ($zone eq "attc") { $height = $side_length * 5 / 12;  $vert_offset = $CSDDRD->[112] + $CSDDRD->[113] + $CSDDRD->[114];};
			$height = $height + $vert_offset;
			& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#VERTICES", 1, 1, "0 0 $vert_offset");
			& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#VERTICES", 1, 2, "$side_length 0 $vert_offset");
			...
			if (($zone eq "main") && ($record_indc->{"bsmt"} || $record_indc->{"crwl"})) {& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#SURFACE_ATTRIBUTES", 1, 6, "  6, Base-6        OPAQ  FLOR  WALL-1       ANOTHER        ");};
			if (($zone eq "bsmt") || ($zone eq "crwl")) {& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#SURFACE_ATTRIBUTES", 1, 5, "  5, Top-5         OPAQ  CEIL  WALL-1       ANOTHER        ");};
			if (($zone eq "main") && $record_indc->{"attc"}) {& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#SURFACE_ATTRIBUTES", 1, 5, "  5, Top-5         OPAQ  CEIL  WALL-1       ANOTHER        ");};
			if ($zone eq "attc") {& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#SURFACE_ATTRIBUTES", 1, 6, "  6, Base-6        OPAQ  FLOR  WALL-1       ANOTHER        ");};
			& simple_replace ($hse_file->[$record_extensions->{"$zone.geo"}], "#BASE", 1, 1, "6 0 0 0 0 0 $CSDDRD->[100] 0");
		}
