...
sub afn_node {
	...
	$type = {'int_unk' => '0', 'bnd_wind_ind' => 3}->{$type};
	if ($type !~ /[0|3]/) {&die_msg('Bad AFN node condition type', $type, $coordinates);}
...
sub afn_degrees {
	...
	if ($AFN_rotation !~ /[0|2|4|6]/) {&die_msg('Bad AFN surface - determining rotation index', $surface, $coordinates);}
