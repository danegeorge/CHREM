...
my @FSA_desired; # declare an array to store the house names or part of to look
...
MAIN: {
	sub main () {
		...
		RECORD: while ($CSDDRD = &one_data_line($CSDDRD_FILE, $CSDDRD)) {	# go through each line (house) of the file
			...
			foreach my $FSA (@FSA_desired) {
				# it matches, so set the flag
				if ($CSDDRD->{'postal_code'} =~ /^$FSA/) {$desired_house = 1};
			};
