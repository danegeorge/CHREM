...
$file=$_[0];
	...
	open(OUT,">2007-10-31_EGHD-HOT2XP_dupl-chk_A-files_region_qual_pref-SD-#$file.csv")||die("can't open datafile:$!");
...
for ($i=0;$i<=$#{$tmp[$p]};$i++){
	&CATEGORY;
#TIER1
	if ($info[$p][$i][1]==5){				#New vintage
		push (@{$tmp[$p+1]},[@{$tmp[$p][$i]}]);
		splice (@{$tmp[$p]},$i,1);
	}
	elsif ($info[$p][$i][6]>=6){				#Floor Area
		push (@{$tmp[$p+1]},[@{$tmp[$p][$i]}]);
		splice (@{$tmp[$p]},$i,1);
	}
...
#TIER2
	elsif (((($file==1)||($file==2)||($file==4))&&($info[$p][$i][1]==4))||(($file==4)&&($info[$p][$i][1]==1))){	#Eastern & QC & prairies 80's vintage; prairies early vintage
		push (@{$tmp[$p+2]},[@{$tmp[$p][$i]}]);
		splice (@{$tmp[$p]},$i,1);
	}
#	elsif ((($file==1)||($file==2)||($file==4))&&($info[$p][$i][6]==3)){
#		push (@{$tmp[$p+2]},[@{$tmp[$p][$i]}]);
#		splice (@{$tmp[$p]},$i,1);
#	}
